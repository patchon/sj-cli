...
- Only shows non-cancelled bookings, from today on — unless `--since` (below) widens the window and asks for cancelled bookings too. Pagination: fetches all pages from the bookings API. Read-only.
...
**`--since DATE`** (modifier, `--list-bookings` only, §5.8): widens the window's start to `DATE` instead of today, and implies cancelled bookings — the fetch's `includeCancelledBookings` flag, off by default, goes on. `DATE` is one of three forms: a date (`2026-06-01`); an ISO week (`W38`, or `2026-W38` for another year — the same bare-week rule as the `dates` grammar and `--cancel-date`, §4.3), resolving to that week's Monday; or an offset back from today, `Nd` (days) or `Nm` (calendar months — the day clamped to the target month's last, so 31 March minus one month is 28 February). The unit letter is case-insensitive; only `d` and `m` are offsets (no weeks or years — a bare week already means `W38`, and an offset `w` would be confusing), and the count must be at least 1. `--since` reaches backwards, so a value that resolves to a date after today is a usage error — including a bare week early in the year, which resolves into next year's numbering and so lands in the future; the error names the resolved date and points at the explicit `YYYY-Www` form. The value is parsed and checked up front (`parse_since`, `dates.py`), before any API call, the same validate-first contract as `--cancel-date`.

A cancelled journey never sits in a booking's `journeys` list: the API moves it into a sibling `cancelledJourneys` list of the same shape (segments, `departureDateTime`, stations, …). Once every journey of a booking is cancelled, `journeys` is empty and the booking rendered nothing before `--since` existed. With `--since`, those legs are rendered too, each with a further `cancelled` column after the booking number:
...
| `--since DATE` | Modifier for `--list-bookings` only (§5.5): list from `DATE` instead of today, and include cancelled bookings (their legs, read from `cancelledJourneys`, marked `cancelled`). `DATE` is a date, an ISO week, or an offset back from today (`90d`, `6m`); validated up front, a value that resolves to the future is a usage error. Usage error alone or with any flag other than `--list-bookings`. | Only SMS on first login. |
