...
- `pinfo()` plain message, `pdim()` dimmed context, `spinner()` progress with a dim `✓`/`✗` trail line (or none with `trail=False`), `print_day_header()` / `print_day_note()` / `print_leg_lines()` for cards, `week_headers()` to group them under `W<n>` week lines, `indented()` to nest everything printed inside a block under a day header.
