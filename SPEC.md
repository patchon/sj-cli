...
| `seat_preference` | Optional (absent means SJ assigns the seat); required for `--change-seat-date` / `--change-seat-booking` (§5.4). Either the literal `"ask"` (prompt for every leg) or a list of ranked vocabulary words: `window`, `aisle`, `table`, `solo`, `easy access`, `no animals`, `forward`, `backward` — an earlier word in the list outweighs every later one combined. Words are normalised in place (lower-cased, inner whitespace collapsed to one space). Rejected: a value that is neither `"ask"` nor a list of strings (`seat_preference must be "ask" or a list of: aisle, backward, easy access, forward, no animals, solo, table, window`); an empty list (`seat_preference is empty — omit the key to let SJ assign the seat`); an unknown word (`seat_preference: unknown "middle". Valid words: …`); a word listed twice (`seat_preference lists window twice`); and the two contradictions `window`+`aisle` and `forward`+`backward` (`seat_preference cannot ask for both window and aisle`). |
...
**`--seat-details`** (modifier, `--list-bookings` only, §5.7): appends the assigned seat's characteristics to the seat cell, e.g. `carriage 3 seat 34 · window, table, forward`, in the same vocabulary `seat_preference` uses (§4.3) plus `forward`/`backward` (the API's `IDR`/`ODR` codes). The bookings-list endpoint's own `seatProperties` are always empty, so this reads them from that leg's seat map instead — one extra `get_seatmap` request per leg, hence opt-in. Only fetched for a leg that has not yet departed and carries `seatMapAvailable`/`seatMapSearchId`; a map fetched for one leg is reused if another leg's `seatMapSearchId` matches it. A map that will not load, has no assigned seat, or carries no recognisable property code leaves that leg's seat cell as the plain `carriage N seat M` and is counted; the run closes with one aggregated `! seat details unavailable for N leg(s)` rather than one warning per leg:
...
```
 fri 28 aug 2026   Linköping Central ⇄ Stockholm Central
   ...
   ← 17:22 – 21:53   4h 31m   X 2000 543   carriage 3 seat 19 · aisle, backward           2 klass Lugn   EPPE0XKQ
```
...
| `--seat-details` | Modifier for `--list-bookings` only (§5.5): append each not-yet-departed leg's assigned seat characteristics (window/aisle/table/forward/backward) to its seat cell — one extra `get_seatmap` request per eligible leg. Usage error alone or with any flag other than `--list-bookings`. | Only SMS on first login. |
