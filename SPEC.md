...
| `punctuality.py` | Was a past leg late (`--delays`)? The pure verdict rules against the `[delays]` threshold (text, claim flag, a `tone` the renderer colours by and the source's label), the four external source adapters and the `lookup()` cascade. Stateless; no SJ client (source 1 is passed in as a callable), nothing raises out. |
...
Mode dispatch:
  ...
  --request-compensation NUM → account + bookings + eligibility lookup → delay check per ticket → pick → identity number → confirm → file (§5.11)
  --list-claims           → bookings in the pass window → one lookup per departed booking → claimed tickets with claim numbers (§5.11)
  (--dry-run modifies --book / --book-journey / --cancel-* / --change-seat-* / --upgrade-class / --request-compensation: preview only, nothing mutated)
  --book                  → booking loop:
...
| `[delays]` | Optional table, validated in every mode (absence is fine, defaults apply): `compensation_minutes` an integer ≥ 1 (default 60 — Swedish law sets 20 minutes on routes under 150 km and 60 above); `trafikverket_key` an optional non-empty string (free registration at data.trafikverket.se). There is no on-time tolerance to configure: a verdict reports the signed minutes (§5.5), so the compensation note is the only threshold — a leftover `on_time_minutes` is rejected with a migration hint (`on_time_minutes was dropped: a verdict reports the signed minutes, "on time" means exactly on the minute`), the way `date_start`/`date_end` are. Read by `--list-bookings --delays` only (§5.5). |
...
The closing status is green only for a real run that actually wrote a seat (`N seat(s) changed`); a real run that matched something but changed nothing — every segment already held its best seat, or none were changeable — closes dim (`nothing changed`). A dry run closes dim too, counting its proposals: `dry run · 1 seat(s) would change`, or `dry run · nothing to change` when no leg would move — `"ask"` mode proposes nothing to count (it only reports the current seat and how many are free), so it always closes `dry run · nothing to change`. A dry run never turns the line green, even when its per-leg lines say `would take …`, because nothing is actually written. Nothing matching at all closes red (`no bookings matched`). As everywhere in this feature (§10.2): a seat preference is never worth failing a booking over — a map that will not load, no free seat to choose from, or a rejected PATCH each degrade to keeping the seat SJ assigned and print a `!` line rather than aborting; the exit code is affected only by an unresolved `--change-seat-booking` number or an unexpected failure while applying a change (§5.9).
...
- Only shows non-cancelled bookings, from today on — unless `--since` (below) widens the window, and/or `--show-cancelled` (below) asks for cancelled bookings too; the two compose. Pagination: fetches all pages from the bookings API. Read-only.
- Opens with the header box (`operation   listing bookings` + account/travelpass/holder), then the week lines and their day cards; the bookings fetch is a silent spinner whose text counts the pages as they arrive — `fetching bookings · 20 of 42`, the total from the response's `totalCount` — while pages remain (one page shows no count). The closing status line (dim ●, dim text — a listing changes nothing) counts days and bookings (leg count omitted — the legs are visible in the cards), plus `N in the past` when applicable. Elsewhere, progress steps that matter leave a dimmed trail line when done — `✓ searching outbound at 06:59`, or `✗ …` if the step raised — so logs show where time went or where a step failed. When stdout is not a TTY only the trail line is printed.
...
- The card renderer (`day_header`, `leg_lines` in `output.py`) is shared by list, book, dry-run and cancel output; columns that are empty on every row of a card are omitted, so book/list cards show train · seat · class · flexibility · number while dry-run cards show train · class · flexibility/note. Three further cells follow the booking number, in this order, when a modifier fills them: `delay` (`--delays`, §5.5), the source it came from (`delay_source`, parenthesised and dim) and then `cancelled` (`--show-cancelled`, §5.5). Travel passes render as cards in the same grammar (`print_travelpasses`): bold pass name + card number as the header — an all-digit number grouped in fours (`1234 5678 9012 3456`) rather than one run of digits, any other shape (an em dash placeholder, a future alphanumeric format) printed as it came — then dim-labelled facts (holder, valid with days left, price from the receipt); unknown facts are omitted, and a `● N travel pass(es)` status line closes the mode.

**`--seat-details`** (modifier, `--list-bookings` only, §5.8): appends the assigned seat's characteristics to the seat cell, e.g. `carriage 3 seat 34 · single, window, table, forward`, in the same vocabulary `seat_preference` uses (§4.3) plus `forward`/`backward` (the API's `IDR`/`ODR` codes). The bookings-list endpoint's own `seatProperties` are always empty, so this reads them from that leg's seat map instead — one extra `get_seatmap` request per leg, hence opt-in. The spinner counts the legs as they are handled — `fetching seat details · 5 of 38` — while legs remain (a single leg shows no count); like every live count it never reaches a trail line. The assigned seat is first looked up by carriage and seat number in that map's carriage layout, the same join `free_seats()` does, so `single` can be computed from the layout's geometry; when the seat cannot be found there (an unfamiliar map shape), rendering falls back to the assigned entry's own `carriageSeatProperties` codes alone — that path can never report `single`, since no code carries it. Only fetched for a leg that has not yet departed and carries `seatMapAvailable`/`seatMapSearchId`; a map fetched for one leg is reused if another leg's `seatMapSearchId` matches it. A map that will not load, has no assigned seat, or carries no recognisable property code leaves that leg's seat cell as the plain `carriage N seat M` and is counted in the warning; the run closes with one aggregated `! seat details unavailable for N leg(s)` rather than one warning per leg.

When `seat_preference` (§4.3) is a ranked word list, a leg whose seat map holds a strictly better free seat gets a further `· could take <n> · <words>` appended, naming that seat's number and its own words in the same vocabulary — e.g. `carriage 3 seat 21 · aisle, backward · could take 47 · single, window, forward`, so it is obvious at a glance which tickets are worth re-seating (`--change-seat-date`/`--change-seat-booking`, §5.4, apply the change). "Better" is judged by the exact ranking `best_seat` uses to choose a seat (§4.3, §5.4) — never by whether the two seats merely differ, since `best_seat` is best-effort and can return the lowest-numbered free seat even when it satisfies no wish at all. The hint needs a wish list to judge "better" against, so it stays silent whenever `seat_preference` is `"ask"` or absent (`[search_parameters]` itself may be entirely absent from a config valid for `--list-bookings`, which does not require it) — and, like the plain seat cell, silent for a leg already on its best seat, one already departed, or a map that will not load. A leg whose map offers no selectable seat at all gets `· nothing free in this class` instead of a suggestion, and one whose map is locked (`_seats_locked`: `canChangeSeat: false` or `hasDeparted`, the same test both write paths apply) gets `· cannot be changed` — the API's `seatsPossibleToSelect` is filtered to the ticket's inventory class, so this means the ticket's carriage class is full, not the train (seen live 2026-09-09: a calm-carriage ticket, every carriage listed with an empty list); the note needs the assigned seat to be found in the layout, an unfamiliar map stays silent:
...
**`--since DATE`** (modifier, `--list-bookings` only, §5.8): widens the window's start to `DATE` instead of today — nothing else changes; pair it with `--show-cancelled` (below) to see cancelled bookings that far back too. `DATE` is one of three forms: a date (`2026-06-01`); an ISO week (`W38`, or `2026-W38` for another year — the same bare-week rule as the `dates` grammar and `--cancel-date`, §4.3), resolving to that week's Monday; or an offset back from today, `Nd` (days) or `Nm` (calendar months — the day clamped to the target month's last, so 31 March minus one month is 28 February). The unit letter is case-insensitive; only `d` and `m` are offsets (no weeks or years — a bare week already means `W38`, and an offset `w` would be confusing), and the count must be at least 1. `--since` reaches backwards, so a value that resolves to a date after today is a usage error — including a bare week early in the year, which resolves into next year's numbering and so lands in the future; the error names the resolved date and points at the explicit `YYYY-Www` form. The value is parsed and checked up front (`parse_since`, `dates.py`), before any API call, the same validate-first contract as `--cancel-date`.

**`--show-cancelled`** (modifier, `--list-bookings` only, §5.8): also lists cancelled bookings — the fetch's `includeCancelledBookings` flag, off by default, goes on. A cancelled journey never sits in a booking's `journeys` list: the API moves it into a sibling `cancelledJourneys` list of the same shape (segments, `departureDateTime`, stations, …). Once every journey of a booking is cancelled, `journeys` is empty and the booking rendered nothing before `--show-cancelled` existed. With `--show-cancelled`, those legs are rendered too, each with a further `cancelled` column after the booking number — pair it with `--since` to reach back past today at the same time:

```
W22
  mon 01 jun 2026   Göteborg Central ⇄ Stockholm Central   past
    → 06:59 – 10:04   3h 05m   X 2000 520   carriage 3 seat 34   2 class calm   FULLFLEX   ABCD1234   cancelled
    ← 17:22 – 20:28   3h 06m   X 2000 543   carriage 3 seat 22   2 class calm   FULLFLEX   ABCD1234   cancelled

● 1 day(s) · 1 booking(s) · 2 in the past · 2 cancelled
```
...
**`--delays`** (modifier, `--list-bookings` only, §5.8): looks up, live, whether each past leg arrived on time, and puts a further column after the booking number — `on time`, `+11 min`, `-1 min`, `+64 min · claim compensation`, `train cancelled · claim compensation`, or `no data` — followed by the source the figure came from, in dim parentheses (`(sj.se)`, `(trafikverket.se)`, `(tagradar.nu)`, `(tågexperterna.nu)`; nothing after `no data`). There is no tolerance band: `on time` means the train arrived on its planned minute, and every other arrival shows the signed difference, early (`-1 min`) as honestly as late. Nothing is stored between runs. SJ's booking data never records an arrival, so the tool asks five sources in order and takes the first answer:
...
| 2 | Trafikverket's open API, only with `[delays].trafikverket_key` | exact | about four days |
...
A leg older than five days goes straight to source 5. Our stop is the one whose planned arrival matches the leg's to the minute. Minutes late are whole clock minutes, seconds dropped on both sides rather than rounded (planned `19:02`, arrived `20:06:33` is 64 minutes late, as every official figure reports it), so a 59:31 delay is never a claimable 60. When only the final-stop figure exists the cell says so and claims only cautiously: `final stop on time · likely`, `final stop +20 min · likely`, `final stop +78 min · likely, verify`, and a cancellation is hedged the same way — `final stop cancelled · likely, verify` — because the summary's cancellation flag speaks for the train's final stop, not necessarily for our own. Source 5 is exact only when the summary's planned final-stop arrival equals the leg's planned arrival to the minute; then the final stop *is* our stop and the cell reads like the others. The one threshold is `[delays].compensation_minutes` (§4.3): from it the cell adds `· claim compensation` (a final-stop figure `· likely, verify`). Booking-cancelled legs (`--show-cancelled`) and future legs are never looked up. The spinner counts the legs — `looking up delays · 5 of 38` — like seat details; a source that fails or answers nothing is skipped silently, an unexpected error is one aggregated `! delay lookup failed for N leg(s)`, and the listing never fails on it. The one failure that is *not* silent is a Trafikverket key the service rejects (HTTP 401, or a body carrying `RESPONSE.RESULT[].ERROR` with `SOURCE "Security"`): it is the user's to fix, so it prints `! trafikverket rejected the key · check [delays].trafikverket_key or remove it` once per run — before the aggregated line if both apply — and the source is then skipped for every remaining leg without a further request, the cascade carrying on to the next source as usual. An `ERROR` from any other `SOURCE` is our own query's fault and passes quietly like anything else. The footer adds `· N to claim`, counting exact claim verdicts and final-stop `verify` ones. The cell is coloured by what it means, not by the row: green for `on time`, yellow for signed minutes, and for a claim the figure yellow with the note (`· claim compensation`, or a final-stop `· likely, verify`) in **orange** — all at full strength, since a past row is dim throughout and the verdict would otherwise disappear into it. `no data` and the source label stay dim. Orange has no bright ANSI slot, so it is the one place the tool leaves the terminal's own palette (`ORANGE = "38;5;208"` in `output.py`, §10.2).

```
W37
  thu 10 sep 2026   Göteborg Central ⇄ Stockholm Central   past
    → 06:59 – 10:04   3h 05m   X 2000 520   carriage 3 seat 51   2 class calm   FULLFLEX   ABCD1234   +118 min · claim compensation   (tagradar.nu)
    ← 17:22 – 20:28   3h 06m   X 2000 543   carriage 3 seat 27   2 class calm   FULLFLEX   ABCD1234   on time                        (sj.se)
...
W38
  tue 15 sep 2026   Göteborg Central → Stockholm Central
    X 2000 520 · ERU0HWB2 · holds 2 class
      ...
      i upgraded to 2 class calm · new booking ZSVV7EML
...
| `--book-journey` | Book one journey interactively (§5.10): date, from, to and a return are asked (config values are the defaults), each leg's departures are listed to pick from with the class the pass would get; a departure overlapping a ticket you hold on the same route can be picked to replace it (a pass-free probe first checks SJ sells a seat), one `book?`/`cancel and book?` confirmation, then the booking — a replaced ticket's journey is cancelled right before its leg is booked. Terminal only, dry run included. | Yes (every prompt; also per leg when `seat_preference = "ask"`). |
| `--request-compensation NUM` | Request delay compensation for one ticket of a booking (§5.11): the eligible tickets and their delays are shown, one is picked, the personal identity number comes from `[compensation]` or a prompt, one confirmation, then the claim is filed with Swish as the payout. Terminal only; `--dry-run` shows and files nothing. | Yes (the ticket when several, the identity number when not configured, the confirmation). |
| `--list-claims` | List the compensation claims SJ holds on your bookings (§5.11): one lookup per departed booking in the pass window, each claimed ticket with its claim number. No status exists. | No. |
...
| `--delays` | Modifier for `--list-bookings` only (§5.5): look up live whether each past leg arrived on time, show the signed minutes when it did not, and flag delays long enough to claim compensation for (`[delays].compensation_minutes`, §4.3); a final-stop indication when only that survives, and the source named after every verdict. Composes with `--since`, `--show-cancelled` and `--seat-details`. Usage error alone or with any other flag. | Only SMS on first login; the external sources need none. |
...
|---|---|
| 0 | Success. |
| 1 | Any failure: config or auth error; a `--book` run in which any day's checkout failed or errored; a `--cancel-*` run in which any cancellation was refused by the API, declined at a prompt, or (`--cancel-booking`) the number was not found; a `--change-seat-*` run in which (`--change-seat-booking`) a named number was not found; an `--upgrade-class` run in which any leg ended with no ticket, any release failed or was left unconfirmed, a probe raised, the confirmation was declined, or there was no terminal to ask at (§5.6); a `--book-journey` run that was refused (no terminal), aborted at a prompt, declined at `book?`/`cancel and book?`, found no departures, could not fetch the station list, could not create the booking, whose checkout failed, in which the outbound's held ticket could not be released, or in which any leg's held ticket was released and nothing booked back (§5.10); a `--request-compensation` run that was refused (no terminal, no mobile number), aborted or declined, found no eligible ticket, or whose lookup or any claim call failed (§5.11); a `--list-claims` run in which the bookings fetch or any lookup failed. A day with no offer, a `--cancel-date` date with nothing to cancel, a segment for which `--change-seat-*` found no free seat to choose from or whose write was rejected (kept as SJ assigned, reported with `!`, §5.4), and an `--upgrade-class` leg that was re-booked into a lower class than asked for (a ticket exists) are not failures. |
...
One search (with the return date when asked), then per leg a day header and a pick list — `HH:MM → HH:MM   duration   train   class`, the class being what the pass would get on that departure (`comfort_class`, a fallback marked `fallback`, or `—` marked `no seats`, which cannot be picked) — that column is what the search reports, and the offer step can still fall further down the class chain (§7.2), said as `! outbound class fallback: 2 class calm → 2 class`. Departures already gone are dropped before the lists are built and counted under the day header (`2 already departed`) — the clock is read after the questions, so a train that left while they were being answered is gone from the list too; a leg with nothing left ends the run (`● no departures left for Göteborg Central → Stockholm Central today` when they have all gone, `● no departures found for Göteborg Central → Stockholm Central …` when the search itself came back empty — `today` or `on 2026-09-14` by the leg's date), exit 1. The highlighted row is the enabled departure closest to `time_leave` (`time_return` for the return — 17:00 when the config omits it, which it may when `roundtrip = false`); ↑↓ (Tab/Shift-Tab) or a typed row number move it, Enter picks, Esc aborts. A row that cannot be picked is never highlighted: it is drawn dim, the arrows pass over it (wrapping), a typed number pointing at one shows its reason in the footer and leaves the highlight where it was, and a default that lands on one starts on the next selectable row instead. When every row is refused the list still opens — with the highlighted row's reason in the footer, the hint line's `· Esc aborts` appended to it, so the refusal is visible and the way out is still on screen. The pick's offers are read at once; a departure without a 0-price offer is said (`! no 0-price offer at 05:29 · pick another`), disabled, and the list asked again with the closest enabled row highlighted — pick after pick that can leave every row refused, and Esc is the way out of that list as of any other. A ticket the account already holds is named only on the rows it affects — there is no summary line before the list, the row is where the refusal is decided. Two notes: a row that *is* a held train — a held segment leaving at the same instant on the same train (brand + number) or the same route — reads `already booked in 3HT2NEIL` and is refused as `this journey is already booked in 3HT2NEIL · pick another`; only when no held segment is that train does a plain time intersection apply, reading `overlaps 3HT2NEIL · Göteborg Central → Stockholm Central 04:01–08:38` and refused as `overlaps booking 3HT2NEIL · pick another` (the held ticket's own route is always named in front of its times, this list's route included, and a night train that departed the evening before counts as well). Both notes stand in place of the `fallback`/`no seats` one; the `already booked` row is refused whatever class the search reports (the class column keeps what the search said), and so is an overlap with a ticket on *another* route — almost always a mistake, never a change of train. An overlap with a ticket on the list's **own** route is a change of train, and may be picked: before the list opens, one search on the same route and date **without** the pass (a pass search hides availability on a departure overlapping a held ticket, §5.6, so its class column cannot be trusted there) re-finds every such departure and reads its offers — `✓ checking seats on 2 departures overlapping 3HT2NEIL` — and a row SJ sells a seat on reads `replaces 3HT2NEIL · 04:01–08:38` in the first class of the config's chain it sells (`comfort_class`, then the fallbacks when `allow_class_fallback`), enabled; one with no seat for sale is a plain refused `no seats`; one the pass-free search did not show keeps the `overlaps …` note and stays refused — an unknown answer never releases a ticket. A probe that fails is a note (`✗ checking seats …`, `! could not check those seats (…) · the overlapping rows stay refused`) and the run goes on as if nothing had been asked. The probe proves only that SJ *sells* a seat: the pass's 0-price offer is a separate quota, known only after the release, so a replacing pick reads no offer at pick time (the pass search has none until the held journey is released) — its card and the confirmation say so. SJ itself does not refuse a pure time overlap (it has sold a ticket three minutes into another one), so refusing the other-route overlap and the unprobed one here is this tool's rule, not SJ's. When SJ's own offers response names a double booking the pick prints `! outbound: SJ reports a conflict with booking 3HT2NEIL` and stands — the line comes from `resolve_offer`, so `--book` prints it too. A failed bookings fetch is only a note (`! could not check existing bookings: …`). There is no duplicate check — the refused rows are the whole guard; a day `--book` already covered can be booked again.

Then the chosen leg(s) as day cards (one per date; a replacing leg's row carries the note `replaces 3HT2NEIL`), under them one `!` line per replacing leg — `! outbound replaces booking 3HT2NEIL · its 04:01 journey is cancelled first, the pass offer on 05:29 is only known after that` — and:

- `--dry-run`: `● dry run · nothing booked` (dim), exit 0; `● dry run · nothing cancelled, nothing booked` when a leg replaces a held ticket — the probe ran, nothing was released.
- `? book? [y/N]:` — `? cancel and book? [y/N]:` when a leg replaces a held ticket; one confirmation covers the run, as in `--upgrade-class` (§5.6) — no: `● booking aborted, nothing was booked` (red), exit 1.
...
A replacing leg is `--upgrade-class`'s step (§5.6) inside that write path, in leg order: `_release_leg` cancels exactly the held journey (one `serviceIdentifier`, the booking's other journeys kept — `✓ releasing this journey from booking 3HT2NEIL`), then at once a pass search on the route and date, the picked departure re-found in it by time and train (never the closest to a config time), its 0-price offer resolved (a class fallback said as `! outbound class fallback: …`), and the leg added to the same cart as any other. The pass cannot hold two overlapping tickets, so the order is fixed: release, then book. Outcomes per leg: a release that fails leaves the ticket intact (`! could not release the ticket: …`), one whose PATCH landed but whose confirmation did not leaves the booking in a pending cancellation (`! cancellation started but not confirmed: …`, `resolve it with: sj-cli --cancel-booking 3HT2NEIL`) — on the outbound either closes red (`● nothing was booked`, the return not attempted), on the return the outbound is booked alone (`! return leg not booked, booking outbound only`). A released leg that then gets no ticket — the pass search no longer shows the departure, the pass has no 0-price offer left on it (`! the travel pass has no offer left on this departure`), or the add fails — prints `! no ticket for this leg: the old one is cancelled and nothing was booked back` and `recover: book it again with sj-cli --book-journey, or on sj.se`, and the run exits 1 whatever else it booked: `● nothing was booked · the outbound has no ticket`, or `● booked 3HT2NEIL · the return has no ticket` after a checked-out outbound.
...
### 5.11 Request-compensation mode

```bash
sj-cli --request-compensation 3HT2NEIL             # look up, check the delay, ask once, file the claim
sj-cli --request-compensation 3HT2NEIL --dry-run   # the eligible tickets and their delays, nothing filed
```

Delay compensation for one ticket of a booking, filed through the same API as sj.se's `ersättning vid försening` form. The form is public: no bearer token, its own subscription key (the third front-end key in `client.py`), and the state of a claim in progress lives in the `delayCompensationToken` each step returns; nothing is filed before the last call. The value takes exactly one booking number (`--cancel-booking`'s grammar, upper-cased; a comma list is refused with `give one booking number: a claim is for one ticket at a time`, a date with the usual hint), validated before auth. A real run needs a terminal on stdin (it asks, and files in the user's name): `● not a terminal · --request-compensation asks before filing a claim`, exit 1, before any request; a dry run asks nothing, so it runs from a pipe.

The steps, after the header box: the account's session (`get_customer_session`, silently — the mobile number, `privatePhoneNumber` then `loginPhoneNumber`, and the name; none → `● no mobile number on your SJ account · add one under account settings on sj.se`, exit 1), the bookings listing (silently; a failed fetch is only a `!` note — the ticket's card then shows the lookup's own facts, without class or seat), then `✓ looking up booking 3HT2NEIL` (`create_compensation_token`: the account's e-mail as the form's order security, the pass's card number and its type — the pass name without its `SJ ` prefix, `Årskort Silver` — and the booking number). Its answer names the eligible tickets (`eligibleOrderItems`, each with its `ticketNumber`, `3HT2NEIL-001`) and any claim SJ already holds (`existingServiceRequests`, each printed as `! SJ already has a claim on this booking: 1-…` — the run goes on, SJ has the final say); none eligible → `● no ticket on 3HT2NEIL is eligible for compensation`, exit 1. Every eligible ticket then gets the `--delays` lookup (§5.5, same sources, thresholds and key, under `looking up delays · n of m`) and is shown as a day card with the delay and source columns of the listing, the ticket number in the booking-number column.

- `--dry-run` stops there: a `!` line per ticket not shown as worth claiming (below), then `● dry run · nothing requested` (dim), exit 0.
- With several tickets a pick list (`? ticket [n]:`, rows `date   HH:MM → HH:MM   train   ticket   verdict (source)`) opens on the first row worth claiming; Esc aborts. A single ticket is taken as is.
- A ticket the lookup does not show as claimable is a warning, never a refusal: `! this train does not seem to have arrived late (+3 min, sj.se) · SJ may refuse the claim`, or `! no arrival data was found for this train · SJ may refuse the claim` for `no data`.
- The personal identity number: `[compensation].personal_identity_number` when set (§4.3), otherwise `? personal identity number:` until it parses (ten or twelve digits, `-`/`+`/space optional, a real past date, the Luhn digit; the reason on a `!` line — `the check digit does not match`, `not a date`, `empty`), Ctrl-D aborts. Normalised to `YYYYMMDD-NNNN`, shown masked (`19850315-****`), redacted in logs.
- The facts block (`ticket`, `train`, `delay`, `contact`, `payout` = `Swish <mobile>`, `identity`), then `? request compensation? [y/N]:` — no, Esc or Ctrl-D anywhere: `● aborted, nothing requested` (red), exit 1.
- yes: `✓ sending travel details` (`put_compensation_travel_details`, the one ticket number, no expenses — a multipart form with a JSON part named `data`, as the site sends it), `✓ sending contact details` (name, e-mail, mobile), `✓ registering the Swish payout` (`create_swish_payout`: identity number, mobile, token → a Paynova `barId`), `✓ filing the claim` (`confirm_compensation` with that id). A failure before the last call: `● could not request compensation (…) · nothing was filed`, exit 1. A failure on the last call cannot tell whether SJ filed it: `● claim status unknown (…) · check your claims on sj.se before trying again`, exit 1. Success: `● compensation requested · claim 1-…` (green, the `ticketCompensationServiceRequests` numbers), exit 0.

`--list-claims` lists the claims SJ holds on the account's bookings. SJ offers no claims list, only the lookup above, so every active booking in the pass window (pass start to today; `--since` moves the start, as for `--list-bookings`, and adds the `since` header row) with a departed leg is looked up once (`✓ fetching bookings`, then `looking up claims · n of m` live, `✓ looking up claims` as the trail line) and each claimed ticket is shown as a leg row under its day card, the claim number(s) in a column after the booking number: `… ABCD1234   claim 1-100000012345`. A claimed ticket the booking does not show, or an entry of an unknown shape, is a `!` line naming the booking; a lookup that fails is one aggregated `! claim lookup failed for N booking(s): …` and the rest is still shown, exit 1. Footer: `● 3 claim(s) on 2 booking(s)` (dim; claim numbers counted), or `● no claims found`, exit 0. No status is available — SJ's lookup says a claim exists, nothing more. Read-only, no terminal needed, no `--dry-run`; `--since` is the one modifier.

Swish is the only payout (the bank-account form and its reference data are not implemented). The payout requirement the travel-details step reports (`bankAccountInfoRequirement`) is not read: a pass ticket is always `TICKET_NOT_POSSIBLE_TO_CREDIT`, so the Swish step is always taken. Expenses are never claimed.
...
- `pinfo()` plain message, `pdim()` dimmed context, `spinner()` progress with a dim `✓`/`✗` trail line (or none with `trail=False`; the block gets `update(text)` for a live count — pages fetched, legs read, or the day's place in a run that walks selected dates — and the trail keeps the original text), `print_day_header()` / `print_day_note()` / `print_leg_lines()` for cards, `week_headers()` to group them under `W<n>` week lines, `indented()` to nest everything printed inside a block under a day header.
