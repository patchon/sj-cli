...
| `punctuality.py` | Was a past leg late (`--delays`)? The pure verdict rules against the `[delays]` thresholds, the four external source adapters and the `lookup()` cascade. Stateless; no SJ client (source 1 is passed in as a callable), nothing raises out. |
