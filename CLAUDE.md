...
# forward/backward) — one extra request per leg, so opt-in
...
| `seats.py` | Seat selection logic (pure). `SEAT_WORDS` vocabulary + `parse_preference` (config validation), `free_seats()` (joins `seatsPossibleToSelect` with `carriages[].seats[]`, computing `forward` as `seat.reversed == carriage.reversed`), `satisfies()`/`best_seat()` (lexicographic on the wish list, ties on lowest carriage/seat), `describe_seat`/`seat_words` for display. `assigned_seat_words()` renders one assigned seat's own property codes the same way, for `--list-bookings --seat-details` (`IDR`/`ODR` → forward/backward, `SEAT_STD`/unknown codes skipped). No HTTP, no printing. |
...
```bash
./venv/bin/pytest               # ~320 tests, <1s, no network
