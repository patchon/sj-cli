...
- **Dates/times** — never parse API timestamps by hand; use `dates` helpers. Config times are Swedish local. A past `date_start` is not an error: `booking_window()`/`process_date_range(today=…)` start from today (tests pass a fixed `today`).
...
[search_parameters]
date_start = "2026-01-19"         # may already lie in the past: a run starts from today
date_end = "2026-03-20"           # must not have passed
time_leave = "05:29"
time_return = "17:22"
