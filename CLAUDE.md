...
- **Interactive modes need a terminal** — `--book-journey` (dry run included, its prompts are the mode) refuses before any request when stdin or stdout is not a TTY; a real `--upgrade-class` does the same on a non-terminal stdin. The pickers raise `SJError` rather than hang if called without one. A `--book-journey` provisional left by a failed checkout is *not* covered by the `--book` cleanup (that is route-scoped), and the closing line says so.
