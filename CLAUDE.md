...
# Book one journey interactively (date, from, to, return?; pick the train per leg from a list
# that filters as you type; one confirmation). Terminal only; --dry-run asks and lists, books nothing.
sj-cli --book-journey
sj-cli --book-journey --dry-run
...
# List back to a date instead of from today: a date, an ISO week, or an offset back from today
sj-cli --list-bookings --since 2026-06-01
sj-cli --list-bookings --since W38
sj-cli --list-bookings --since 90d

# Also list cancelled bookings (marked); combine with --since to reach back past today
sj-cli --list-bookings --show-cancelled
...
sj-cli --change-seat-date 2026-01-20 --dry-run   # preview: seats it would take (closing counts them), nothing written
...
| Module | Role |
|---|---|
| `cli.py` | Entry point. CLI argument parsing (`allow_abbrev=False`: no prefix aliases), top-level orchestration (`main()` → `_run()` so the HTTP client is always closed; every failure exit ends with a red `●` line via `pstatus`), travel-pass selection (`resolve_travel_pass`: expired passes ignored, the pass covering the booking window wins, prompts only at a terminal, listing/cancel modes never prompt; `--book-journey` takes the single valid pass, or the one chosen at the numbered prompt (no window — the wizard validates the typed dates itself via `pass_validity`), and dispatches to `journey.handle_book_journey` after the header box), `booking_window()` (first/last bookable selected date from today on, None when all are skipped) and date-vs-pass validation on it. `--since` (a `--list-bookings` modifier, `dates.parse_since`) is validated up front like `--cancel-date` (`parser.error` on a bad value, before any auth), the resolved date kept on `args.since_date` and passed to `handle_list_bookings`; the `--list-bookings` header box appends a `since` row with that date when set, since `90d`/`6m` resolve relative to whenever the run happens. `--show-cancelled` (another `--list-bookings`-only modifier, plain `store_true`, composes with `--since`) is passed through as `show_cancelled=` and adds a `cancelled   shown` header-box row when set. |
...
| `client.py` | HTTP client. All SJ API communication via `httpx.Client` (`HTTP_TIMEOUT`: 30 s, 10 s connect). Low-level request methods only — no business logic; every method raises on failure: data endpoints through `_json_or_raise` (error envelope beats the status even behind a 200, empty/non-JSON 2xx is an error), the cancel/revert/confirm methods and the B2C fingerprint POSTs through `_raise_for_failure` (empty/HTML 2xx fine); callers print the cause. `refresh_token()`: dict on success, None when B2C rejects the refresh token, `SJAuthError` on transient failures (the POST opts into the GET retry policy via `RETRY_EXTENSION`). Constants for API base URLs/headers. `RetryTransport`: GET retried on 502/503/timeouts/connection errors/resets (1s/2s/4s); POST/PATCH retried only on connect errors (request never sent); the body is read inside the loop (`_read_fully`: raw bytes, fresh response — a stalled body is that attempt's error, a retried 503 releases its connection). `get_stations()` (public, no token); `resolve_station` passes a UIC code through. `get_bookings(..., include_cancelled=...)` sends `includeCancelledBookings` accordingly (default false, matching the API's own default). |
| `booking.py` | The booking core every write path uses: `search()` (one journey search → its ids), `describe_departure()`, `train_name(node)` (brand + public number of a search leg or a booked segment, "X 2000 520" — the cards' train column and `journey._match_held` read it), `resolve_offer()` (a departure → a `Leg` with the pass's 0-price offer), `Cart` (`add` = create the provisional or add a leg, `finish` = seats → customer → checkout, never raising past checkout); the travel-pass validity helpers `pass_validity`/`is_expired_pass`/`pass_covers` (moved here from `cli` so any mode can check a date against the pass). The bookings fetch: `fetch_all_bookings(…, progress=…)` walks the pages (`nextPage`) and reports `(fetched, totalCount)` after every page that another follows; `fetch_bookings_with_spinner(…, label=…, trail=…, nth=…, unit=…)` shows that as `label · 20 of 42` in the spinner, after the optional `nth=(i, n)` ordinal (`label · day 3 of 28`) that the three date-walking modes pass for their place in the run, with `unit` naming what is being walked (`day` by default, `booking` for `--cancel-booking`, giving `label · booking 2 of 3`) — every mode fetches bookings through it, so a single page (each per-fetch call) never shows a page count, the ordinal is omitted for a lone item, and a trail line keeps the bare label. Then: booking business logic. `process_date_range` (walks `selected_dates()` from `today` on, dropping past days with a note, contains a mid-run refresh failure, a planning or rendering failure to the day) → `process_booking_flow` (duplicate check, which search) → `handle_booking_process` (legs + checkout; the customer PATCH sends back the phone the API put on the provisional, never a placeholder) → `_resolve_leg` (search → select → offers → one alternative; shared by dry-run and book mode). Also cancellation (`handle_cancel_booking(only_date=…)` for `--cancel-date`), listing (`handle_list_bookings`; `seat_details=True` fetches each not-yet-departed leg's seat map — cached per `(booking_id, seatMapSearchId)` — and appends its characteristics to the seat cell, failures aggregated into one `pwarn`, the spinner counting legs — `fetching seat details · 5 of 38` — while legs remain; when `seat_preference` is also a ranked word list, `_seat_hint` appends a further "could take N · words" for a leg whose map holds a strictly better free seat than the assigned one, judged by `seats.rank` — never by seat identity, since `best_seat` is best-effort and a naive identity check would flag a same-or-worse seat as an improvement — says `· nothing free in this class` instead when the map offers no selectable seat (SJ filters that list to the ticket's inventory class) and `· cannot be changed` when `_seats_locked` says SJ refuses a re-seat at all — and stays silent under `"ask"`/an absent preference, same as when `[search_parameters]` itself is missing from a `--list-bookings`-valid config; `since=` (a `date`, from `--since`) widens the window's start to that date instead of today — nothing more; `show_cancelled=` (from `--show-cancelled`, composes with `since=`) is the one that fetches with `include_cancelled=True` and renders a booking's `cancelledJourneys` — the API's sibling list for a cancelled journey, same shape as `journeys` — with a `cancelled` marker on each leg, never fed to the seat-details fetch), stale-provisional cleanup (`cleanup_stale_provisionals(route=…, now=…)`: configured route + older than `STALE_PROVISIONAL_GRACE`), seat selection (`_apply_seat_preference`: chooses and PATCHes seats for every segment of a booking, shared by `--book`'s provisional path and `handle_change_seat`'s confirmed path for `--change-seat-date`/`--change-seat-booking`), and class upgrades (`handle_upgrade_class` for `--upgrade-class`: `_probe_upgrade` (pass-free search + offers → does SJ sell a seat) gates `_upgrade_one_leg` = `_release_leg` (cancel exactly one `serviceIdentifier`) + `_rebook_released_leg` (pass search → `_match_departure` → `find_offer_id` → provisional → seats → customer → checkout), with `_leg_recovery` naming the way back for a leg left unticketed). Shared small parts: `drop_departed(departures, now)` (used by `poll_and_select`, the alternative and the wizard — a departed train is never listed or picked); `offer_conflicts()` (the offers response's `bookingsInConflictForDoubleBooking`, printed by `resolve_offer` as a `!` line); booked cards (`_segment_to_display_row`) take class and flexibility from `productFamily.salesCategoryComfort`/`salesCategoryFlexibility`, `split_product_name` only for a segment without codes. |
...
| `output.py` | User-facing output helpers. `pinfo()`/`pdim()`/`pnote()`/`blank()`, `spinner()` (shares a stdout lock with pinfo so messages never interleave with frames; `trail=False` for silent waits; yields `update(text)`, which redraws the live frame at once — the trail line always keeps the original text), `indented()` nesting, ANSI styling (`style`/`pad`/`visible_len`; auto-off when not a TTY or `NO_COLOR` set), the shared day-card renderer (`day_header`, `print_day_note`, `leg_lines`/`print_leg_lines` — a `cancelled` row key renders as a further column after the booking number, e.g. `… ABCD1234   cancelled`, picked out by name (not position) alongside `booking_number` so a later column insertion can't silently print as the wrong one, and dropped entirely rather than shown as an em dash for an uncancelled row in a mixed listing —, `print_bookings_table`; `week_headers()` groups the day cards printed inside it under bold `W42` ISO-week lines (dim when every card in the week has departed) one level up — the card openers and the table consult it, so a card opener must be called at the block's base indent), `split_product_name` (SJ's Swedish product-family name → the config vocabulary, e.g. `2 class calm` + `FULLFLEX`), the status card (`print_status_card`, auth modes), the header box (`print_header_box`) and the travel-pass cards (`print_travelpasses`: `_group_code` prints a numeric pass number in fours, `1234 5678 9012 3456`) — both built on the same dim-labelled `_fact_line` grammar; `confirm()` (yes/no on stdin); the list pickers `select_filtered` (type-to-filter) and `select_list` (arrows/row number; it never highlights a row `reject` refuses — the arrows skip them, a typed number pointing at one is refused with the reason and the highlight stays, refused rows are drawn dim, and an all-refused list opens on the default row's reason, which then carries the Esc hint) on `termios` cbreak mode — one fixed-height frame under a `?` prompt, clamped to the terminal, redrawn per keystroke, wiped on exit and on Ctrl-C (`_picked`), `_clip` measures visible width and drops styling from a truncated row, injectable `keys` iterator for tests, never inside `spinner()` — and `departure_choice_lines`. |
| `dates.py` | Swedish red-day calendar (no dependency; Easter computed). `skip_reason()` for weekend/holiday skipping. Timezone helpers `parse_api_datetime()` / `to_sweden()` / `sweden_now()` — all API timestamp handling goes through these (Swedish wall-clock for train times/pass validity, aware "now" for past/expiry). The date-selection grammar `parse_date_selection()` (dates, ISO weeks `W43`/`2027-W02`, `start..end` ranges; shared by the `dates` key and `--cancel-date`) and `selected_dates()`/`booking_dates()` (the validated selection, all / from today on). `parse_since()` (the `--since` value: a date, an ISO week, or an offset back from today — `90d`/`6m`, the month form clamped to the target month's last day) resolves to a single date and rejects one that lands in the future. |
| `seats.py` | Seat selection logic (pure). `SEAT_WORDS` vocabulary + `parse_preference` (config validation, incl. the negated `avoid <word>` form of every word), `free_seats()` (joins `seatsPossibleToSelect` with `carriages[].seats[]`, computing `forward` as `seat.reversed == carriage.reversed` and `single` — a seat with no neighbour in a 2+1 carriage, not the same as the `solo` property code — from `_single_seat_numbers()`: within a carriage, the aisle is the widest gap between distinct `ypos` values, seats group by `(rowNumber, side of that gap)`, and a group of one is single), `satisfies()` (`"avoid X"` is exactly `not satisfies(seat, "X")`)/`rank()`/`best_seat()` (lexicographic on the wish list, ties on lowest carriage/seat; `rank()` is the sort key `best_seat()` minimises, exposed so a caller can compare two seats' ranks directly — `--list-bookings --seat-details`'s "could take" hint does exactly that, never by seat identity), `describe_seat`/`seat_words` for display. `assigned_seat()` looks the assigned seat up by carriage + number in the carriage layout so `--list-bookings --seat-details` can render it with the same computed properties as a free seat; `assigned_seat_words()` is its fallback, rendering the assigned entry's own property codes when the lookup fails (`IDR`/`ODR` → forward/backward, `SEAT_STD`/unknown codes skipped, never reports `single`). No HTTP, no printing. |
| `stations.py` | Station lookup for `--book-journey` (pure): `parse_stations` (SJ's public `config/stations` list), `fold` (casefold, diacritics stripped), `StationIndex.match` (exact › name prefix › word prefix › substring › synonym substring, ties by UIC code — the big stations have the lowest; each entry folded and split once at construction) and `.exact` (config default → live entry, lowest code on a tie); `is_train_station` (a station word in the name, or a foreign code — not `74…` — whose last five digits are below the `9xxxx` bus range) — `match` drops matches ranked no better than the best train-looking hit unless they are train-looking too, and shows everything otherwise. |
| `journey.py` | `--book-journey`: `handle_book_journey` — terminal gate, station list, the four questions (`_ask_date` clamps its default up to the pass start and re-asks on garbage/past/outside-the-pass; `_ask_station` refuses the other endpoint and drops the default on the re-ask; `_ask_yes_no`), one `search` (its departures through `drop_departed`, the clock read after the questions so a train that left meanwhile is gone; nothing left ends the run), `_held_segments` (the chosen dates and the day before each, for night trains) → `_warn_held_bookings` (the summary, chosen dates only; warns only — there is no duplicate check) and `_match_held` (`already booked in NUM` for the same instant + train or route, else `_overlap` → `overlaps NUM · route HH:MM–HH:MM`; both rows rejected (`this journey is already booked in NUM · pick another` / `overlaps booking NUM · pick another`), the held ticket's route always named, instants so it works across midnight), `_choose_leg` per leg (prints `N already departed` under the day header; `_departure_rows` + `select_list` + `resolve_offer`; an offer-less pick is disabled and asked again on the closest enabled row; a row without a class can never reach `resolve_offer` — `find_offer_id` would read `None` as "any offer"), summary cards, dry-run stop or `confirm`, then `Cart` (the first `add` caught with a red `●`, a failed return `add` books the outbound alone). Only `output` helpers touch the terminal. |
...
- **One write path** — a ticket is only ever written through `Cart` (`booking.py`): `--book`, the `--upgrade-class` re-book and `--book-journey` resolve their legs differently (`_resolve_leg` by time, `_match_departure` by identity, the user's pick) but all `Cart.add` then `Cart.finish`. Don't call `create_provisional_booking`/`add_offer_to_booking`/`checkout_booking` anywhere else.
...
- **Interactive modes need a terminal** — `--book-journey` (dry run included, its prompts are the mode) refuses before it asks anything, after auth and the travel-pass fetch, when stdin or stdout is not a TTY (exit 1); a real `--upgrade-class` does the same on a non-terminal stdin. The pickers raise `SJError` rather than hang if called without one. A `--book-journey` provisional left by a failed checkout is covered by the `--book` cleanup only when it happens to be on the configured route (that cleanup is route-scoped), so the closing line does not promise it.
...
- **Seats never fail a booking — and never worsen one** — `_choose_seat` keeps a locatable current seat (`seats.assigned_seat`) that ranks at-least-as-good as the best free one (`wish_rank`, ties keep — the `--seat-details` hint's rule), printing `keeping … · nothing free outranks it` and staying out of the PATCH; only an unlocatable current seat falls back to taking the best free one. `_apply_seat_preference` catches everything: a map that will not load, no free seat to choose from, a rejected PATCH all degrade to the seat SJ assigned, with a `!` line. The same helper serves `--book` (provisional path) and the change-seat modes (confirmed path) via `update_seats(..., provisional=...)`.
- **Week lines** — every date-ordered card loop runs inside `week_headers()` (`output.py`), which prints a bold `W<n>` line (dim when every card in the week has departed) when the ISO week changes and nests the cards one level beneath it. Its one invariant: a card opener (`print_day_header`/`print_day_note`, or `print_bookings_table`'s own `_week_break`) is only ever called at the block's base indent — the loop's top level, never inside a further `indented()` — because `_week_break` sets the indent absolutely from the block's base, so an opener fired deeper silently drops that level for the rest of that scope. `process_date_range`'s block spans the whole `process_booking_flow` → `Cart` call tree: do not add a card opener anywhere under it. The block prints no blank lines; callers keep their blank after or between cards. The picker header in `journey._choose_leg` and the two prose leg lists in `handle_upgrade_class` stay outside any block on purpose.
...
seat_preference = ["window", "table", "forward"]  # optional; or "ask" to be prompted; any word can be negated ("avoid table")
```
...
```bash
./venv/bin/pytest               # ~655 tests, <1s, no network
...
All three tools read their configuration from `pyproject.toml`. Tests live in `tests/` and never touch the network: `tests/fakes.py` provides a scripted `FakeClient` (records every API call and customer update), departure/offer builders, a base config (fixed 2026-09 dates for the flow fixtures) and `future_cfg()` (a window that validates whatever today is — use it wherever `verify_cfg` runs); it also serves a station list (`STATION_LIST`/`get_stations()`) and builds a booked segment from the departure whose offer was used; `FakeClient.page_size` makes `get_bookings` page like the real API (`nextPage`/`totalCount`), and `TtyOut` is a captured stdout that claims to be a terminal, so a test can read spinner frames; `tests/conftest.py` holds the autouse fixture (no colour, no sleeping). `tests/test_booking_flow.py` pins the booking flow's API call sequence, return contract and user messages — run it after any change to `booking.py`; `tests/test_dates.py` pins the timezone rules and the `--since` grammar (`parse_since`); `tests/test_client.py` the retry policy; `tests/test_cli.py` the flag contract (including --show-cancelled's) and the --cancel-date/--cancel-booking/--since validate-first parsers; `tests/test_auth.py` the login flow's trail/prompt/SMS-retry behaviour; `tests/test_logout.py` and `tests/test_login_status.py` the auth modes' output and logic; `tests/test_output.py` the shared renderers (cards, header box, status/trail/warn lines, prompts) and the list pickers driven by an injected key iterator (frames, clipping, Ctrl-C wipe, refused rows skipped and dimmed); `tests/test_seats.py` the seat vocabulary, the seat-map join and the ranking (`seats.py`, pure logic); `tests/test_change_seat.py` the `--change-seat-date`/`--change-seat-booking` flow; `tests/test_list_bookings.py` `--list-bookings`/`--seat-details`/`--since`/`--show-cancelled` (fetch gating, caching, aggregated-warning behaviour, the leg counter, cancelled journeys marked and kept out of the seat-details fetch, and the four `since`/`show-cancelled` combinations); `tests/test_upgrade_class.py` `--upgrade-class` — the probe half (pass-free search, same-departure matching, no writes) and the write half (one `serviceIdentifier` per cancel payload, cancel-before-rebook order, the confirmation and terminal gates, and every per-leg outcome including the unticketed one); `tests/test_booking_core.py` the core (`search`, `describe_departure`, `resolve_offer`, `Cart`); `tests/test_fetch_bookings.py` the paged fetch (the progress sequence, single-page silence, the counting spinner's live text and plain trail); `tests/test_stations.py` the folding, the ranking pinned against code order, the exact tie-break and the train-station preference (including the better-ranked match that survives it); `tests/test_journey.py` `--book-journey` — the prompts with scripted answers and the flow's API sequence, defaults, rejects, aborts, dry run and terminal gate, departed trains, the overlap notes (overnight, the held route always named, the return list) and SJ's conflict line, driven by one `Script` reply class whose `select_list` refuses rejected rows like the widget does (`journey.sweden_now`/`booking.sweden_now` frozen); `tests/test_booking_pure.py` the pure helpers (`drop_departed`, the booked row's class/flexibility codes). There is no CI.
