...
- **Seats never fail a booking** — `_apply_seat_preference` catches everything: a map that will not load, no matching seat, a rejected PATCH all degrade to the seat SJ assigned, with a `!` line. The same helper serves `--book` (provisional path) and the change-seat modes (confirmed path) via `update_seats(..., provisional=...)`.
