...
from sj_tool import handle_login_status
...
def _fmt(ts):
    dt = datetime.fromtimestamp(ts, tz=timezone.utc).astimezone(ZoneInfo("Europe/Stockholm"))
...
def test_valid_access_token_card(tmp_path, capsys):
    ...
    # the account lives in the header box (printed by sj_tool), not the card
...
def test_cached_email_reads_profile_offline(tmp_path):
    from sj_tool import _cached_email
...
def test_login_failed_card_from_api_error(capsys):
    ...
    from sj_tool import print_login_failed

    err = SJAuthError("login failed: AADB2C90054 · Fel uppgifter.")
    err.__cause__ = SJAPIError({
        "status": "400", "errorCode": "AADB2C90054", "message": "Fel uppgifter.",
    })
